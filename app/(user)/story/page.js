...
export default function HomePage() {
  ...
  if (status === 'loading') {
    return (
      <div className="flex justify-center items-center min-h-screen bg-gray-50 dark:bg-gray-900">
        <div className="flex items-center gap-2">
          <div className="w-5 h-5 border-t-2 border-r-2 border-gray-600 dark:border-gray-400 rounded-full animate-spin"></div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-200 py-8 px-4 sm:px-6 lg:px-8">
      {/* Welcome Banner */}
      <div className="container mx-auto text-center mb-10">
        <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-gray-700 to-gray-800 dark:from-gray-400 dark:to-gray-300">
          Welcome to WriteHub
        </h1>
        {session?.user && (
          <p className="mt-2 text-lg text-gray-600 dark:text-gray-400">
      ...
      <div className="container mx-auto">
        {/* Error Message */}
        {error && (
          <div className="bg-red-100 border border-red-300 text-red-700 dark:bg-red-900/20 dark:border-red-800 dark:text-red-300 px-4 py-3 rounded-lg mb-6 flex items-center justify-between">
            <div className="flex items-center">
              <FiAlertCircle className="w-5 h-5 mr-2" />
              {error}
            </div>
          ...
          <div className="flex items-center space-x-3 sm:space-x-4">
            ...
            <div className="relative">
              <select
                onChange={handleCategoryChange}
                value={selectedCategory}
                className="block w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 py-2 pl-3 pr-8 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-gray-500 text-gray-800 dark:text-gray-200"
                disabled={loading.fetch}
              >
                <option value="">All Categories</option>
                {categories.map((cat) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {loading.fetch ? (
            ...
            <p className="col-span-3 text-center text-gray-600 dark:text-gray-400">Unable to load stories. Please try again.</p>
          ) : stories.length > 0 ? (
            stories.map((story) => <StoryCard key={story._id} story={story} />)
          ) : (
            <p className="col-span-3 text-center text-gray-600 dark:text-gray-400">No stories found with the selected filters.</p>
          )}
        </div>
      </div>
    </div>
