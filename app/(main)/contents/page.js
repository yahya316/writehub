...
/**
 * Extracted the search params dependent logic
 * into its own client component wrapped in <Suspense>
 */
function ContentsWithParams() {
  const { data: session, status } = useSession();
  const router = useRouter();
  const searchParams = useSearchParams();
  const selectedCategory = searchParams.get('category') || '';
...
export default function HomePageWrapper() {
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 py-8 px-4 sm:px-6 lg:px-8">
      ...
      {/* Wrap contents in Suspense */}
      <Suspense fallback={<p className="text-center">Loading content...</p>}>
        <ContentsWithParams />
      </Suspense>
    </div>
  );
